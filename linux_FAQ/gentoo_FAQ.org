...
#+BEGIN_SRC bash
:let searchengine baidu = "https://www.baidu.com/s?ie=UTF-8&wd=%s"  #设置搜索引擎
:set defaultengine = "baidu" #设置为默认
:map a :tabnew baidu  #修改快捷键

:let searchalias b = "baidu"   #别名，这一项不是必须的，但是在使用t是比较有用
:let searchengine fanyi = "http://fanyi.baidu.com/#en/zh/%s"
:let searchengine zhwikipedia = "https://zh.wikipedia.org/wiki/%s"
#+END_SRC
